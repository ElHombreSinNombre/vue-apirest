    ...
    <div class="w-96 rounded-xl shadow-md p-5 bg-white hover:shadow-lg text-justify">
     <!-- Texto por defecto -->
      <div v-if="searchInput <  3 && loading == false" class="text-center p-0.5 ">
        <div>Introduzca una búsqueda para encontrar diferentes localizaciones</div><i class="fa-2x p-0.5  fa-solid fa-earth-europe"></i>
      </div>
      ...
      <div v-if="countries.length == 0 && searchInput.length >= 3 && loading == false" class="bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded text-center">
        No hay resultados
      </div>
